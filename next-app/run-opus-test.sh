...
if [ -z "$1" ]; then
  ...
  echo "Usage: ./run-opus-test.sh YOUR_API_KEY"
  exit 1
fi

# Make the script executable
chmod +x test-opus-model.js

# Run the test script with the API key
echo "Running Claude 3 Opus model test..."
node test-opus-model.js "$1"

# Remind user to delete the API key
echo ""
echo "Test completed. Remember to remove your API key from your command history:"
