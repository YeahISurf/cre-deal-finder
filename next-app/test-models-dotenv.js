// Test script to run tests on all OpenAI models using dotenv for configuration
...
try {
  ...
  // If neither exists, try to load from default dotenv location
  else {
    require('dotenv').config();
    console.log('Attempted to load configuration from default dotenv location');
  }
} catch (error) {
  console.warn('Warning: dotenv module not found or error loading .env file');
  console.warn('If you want to use a .env file, install dotenv: npm install dotenv');
}
...
// Get default model from environment variable or use all models
...
// If DEFAULT_MODEL is set, only test that model
if (DEFAULT_MODEL) {
  const modelInfo = models.find(m => m.name === DEFAULT_MODEL);
  if (modelInfo) {
    models = [modelInfo];
    console.log(`Testing only the default model: ${modelInfo.displayName}`);
  } else {
    console.warn(`Warning: DEFAULT_MODEL=${DEFAULT_MODEL} is not recognized. Testing all models.`);
  }
}
...
Ensure your response is properly formatted JSON without any unterminated strings or syntax errors.`
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error(`Request timed out after ${API_TIMEOUT}ms`)), API_TIMEOUT);
    });
...
async function main() {
  ...
  console.log(`Using API key from environment variable OPENAI_API_KEY`);
  console.log(`API timeout set to ${API_TIMEOUT}ms`);
